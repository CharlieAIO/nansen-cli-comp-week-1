import type { CommentaryOutput, RoundSummary, TradeDecision } from "@/lib/types";

export class ClaudeService {
  private readonly apiKey = process.env.ANTHROPIC_API_KEY;

  async getTradeDecision(system: string, user: string): Promise<TradeDecision> {
    if (!this.apiKey) {
      return this.getMockTradeDecision(user);
    }
  ...
  private getMockTradeDecision(userPrompt: string): TradeDecision {
    const portfolioMatch = userPrompt.match(/Cash: ([\d.]+) SOL/);
    const cash = Number(portfolioMatch?.[1] ?? 0);
    const tokenMatch = userPrompt.match(/"token_symbol": "([A-Z]+)"/);
    const addressMatch = userPrompt.match(/"token_address": "([A-Za-z0-9]+)"/);
    if (!cash || !tokenMatch || !addressMatch) {
      return { thinking: "Signals are mixed, so I stay patient.", trades: [] };
    }
    const amount = Number(Math.min(Math.max(cash * 0.2, 0.5), cash * 0.45).toFixed(2));
    return {
      thinking: "Mock mode: strongest visible signal gets a measured allocation while keeping powder dry.",
      trades: [
        {
          action: "BUY",
          token_symbol: tokenMatch[1],
          token_address: addressMatch[1],
          amount_sol: amount,
          confidence: 0.67,
          reasoning: "Top-ranked signal in current dataset.",
        },
      ],
    };
  }
